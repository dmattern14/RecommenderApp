...
function RecommendationPage() {
  ...
  const [azureContentId, setAzureContentId] = useState('');
  ...
  useEffect(() => {
    if (!azureContentId) return;

    const fetchAzureRecs = async () => {
      try {
        const response = await fetch('http://09d9f527-9111-461a-827e-c43241612b1b.eastus2.azurecontainer.io/score', {
          method: 'POST',
          headers: {
        ...
        console.log('✅ Azure FULL RAW RESPONSE:', JSON.stringify(data, null, 2));
      } catch (err) {
  ...
  return (
    <div style={{ textAlign: 'center', marginTop: '40px' }}>
      ...
      <div style={{ marginBottom: '40px' }}>
        <label style={{ fontWeight: 'bold' }}>Azure Content ID: </label>
        <select
          value={azureContentId}
          onChange={(e) => setAzureContentId(e.target.value)}
          style={{ padding: '10px', fontSize: '16px', margin: '0 10px' }}
        >
          <option value="">-- Select --</option>
          {azureIds.map((id) => (
      ...
      {(collabContentId || contentContentId || azureContentId) && (
        <>
          ...
          <table style={{ margin: '0 auto', borderCollapse: 'collapse', width: '90%' }}>
            <thead>
              <tr>
                <th></th>
                {[...Array(5)].map((_, i) => (
                  <th key={i} style={{ border: '1px solid gray', padding: '10px' }}>
                    Article {i + 1}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {collabContentId && (
                <tr>
                  <td style={{ fontWeight: 'bold', padding: '10px' }}>Collaborative</td>
                  {collabRecs.map((rec, i) => (
                    <td key={i} style={{ border: '1px solid gray', padding: '10px' }}>{rec}</td>
                  ))}
                </tr>
              )}
              {contentContentId && (
                <tr>
                  <td style={{ fontWeight: 'bold', padding: '10px' }}>Content-Based</td>
              ...
              {azureContentId && (
                <tr>
                  ...
                  {[...Array(5)].map((_, i) => (
                    <td key={i} style={{ border: '1px solid gray', padding: '10px' }}>—</td>
                  ))}
                </tr>
              )}
            </tbody>
